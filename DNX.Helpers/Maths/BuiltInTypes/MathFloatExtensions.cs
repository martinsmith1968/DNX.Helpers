...
namespace DNX.Helpers.Maths.BuiltInTypes
{
    ...
    public static class MathFloatExtensions
    {
        ...
        /// <returns><c>true</c> if the specified minimum is between min and max; otherwise, <c>false</c>.</returns>
        public static bool IsBetweenEither(this float value, float min, float max, bool inclusive)
        {
		...
		/// <param name="inclusive">if set to <c>true</c> min and max are inclusive.</param>
        /// <returns><c>true</c> if the specified minimum is between min and max; otherwise, <c>false</c>.</returns>
        public static bool IsBetween(this float value, float min, float max, bool inclusive)
        {
            return inclusive
                ? (value >= min) && (value <= max)
                : (value > min) && (value < max);
