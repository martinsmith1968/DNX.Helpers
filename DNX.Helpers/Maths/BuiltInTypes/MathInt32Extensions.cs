...
namespace DNX.Helpers.Maths.BuiltInTypes
{
    ...
    /// </summary>
    public static class MathInt32Extensions
    {
        /// <summary>
        ...
        /// <returns><c>true</c> if the specified minimum is between min and max; otherwise, <c>false</c>.</returns>
        public static bool IsBetweenEither(this int value, int min, int max, bool inclusive)
        {
		...
		/// <param name="inclusive">if set to <c>true</c> min and max are inclusive.</param>
        /// <returns><c>true</c> if the specified minimum is between min and max; otherwise, <c>false</c>.</returns>
        public static bool IsBetween(this int value, int min, int max, bool inclusive)
        {
            return inclusive
                ? (value >= min) && (value <= max)
                : (value > min) && (value < max);
